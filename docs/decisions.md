...
| 6 | 2026-09-05 | **Edge Grinder** stays on VGI. `MUSIC_AKL` remains the parked comparison build. Fidelity work here can feed back if and when KC's ear says so. **Superseded 2026-09-06 by row 11.** |
...
| 11 | 2026-09-06 | **Edge Grinder ships AKL, and VGI is retired from its configuration set** (KC), reversing row 6. Recorded here because row 6 is the one this repo quotes and it had gone stale: edge-beeb's own decisions 69, 70 and 73 are where it happened - both CPC tunes on the Arkos replay, this library's `lib/` taken verbatim with `ENV_BASE = 12` and `BASS_MODE = 2`, and VGI dropped. The size case is what it always was and got wider on 2026-09-06: AKL carries both tunes in 9,121 bytes where VGI needed 25,876 and could not fit WON4 at all. `.vgi` v3, measured the same day, would bring that to about 21 KB - still more than twice AKL's, and the fidelity argument was never about size. **Consequence for this repo**: `performance.md`'s VGI row is a comparison, not a description of what the game ships. |
